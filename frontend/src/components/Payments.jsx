...
export default class Home extends Component {
  ...
  retrievePayments() {
    ...
    axios.get("http://localhost:8070/payment/payments").then(res => {
      if (res.data.success) {
        this.setState({
  ...
  onDelete = (id) => {
    axios.delete(`http://localhost:8070/payment/payment/delete/${id}`).then((res) => {
...
TotalPaymentAmount() {
  ...
  render() {
    ...
    return (
            ...
            {this.state.payments.map((payments, index) => (
              <tr key={index}  style={{color:"#00fff2"}}>
                ...
                <td style={{hover:"#ff00b3",textDecoration:'none'}}>
                  <Link to={`/paymentDetails/${payments._id}`} style={{ textDecoration: 'none' }}>
                    {payments.PaymentID}
                  </Link>
                </td>
                ...
                <td>
                  <Link className="btn btn-warning" to={`/editPayment/${payments._id}`}>
        ...
        <Link to="/addPayments"  className="btn btn-success" style={{ textDecoration: 'none', color: 'white' }}><i class="fas fa-plus"></i>&nbsp;&nbsp;Add New Payment Record</Link><br/><br/>
