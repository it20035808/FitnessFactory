...
export default class ExpenseList extends Component {
  ...
  retrieveExpenses(){
    ...
    axios.get('http://localhost:8070/expenses/expenses').then(res => {
      if (res.data.success) {
        this.setState({
  ...
  onDelete = (id) => {
    axios.delete(`http://localhost:8070/expenses/expense/delete/${id}`).then((res) => {
          ...
          <tbody>
            {this.state.expenses.map((expenses, index) => (
              <tr key={index} style={{color:"#dbfc03"}}>
                ...
                <td>
                  <Link to={`/expenseDetails/${expenses._id}`} style={{ textDecoration: 'none' }}>
                    {expenses.ExpenseID}
                  </Link>
                </td>
